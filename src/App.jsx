...
async function aiPolish(text) {
  ...
  const rewrites = [
    [/^do you ever (think about|wonder about|worry about) (.+)/i, (m) => `When you picture our life ahead, where does ${m[2]} sit for you?`],
    [/^do you (still )?(want|wish for|hope for) (.+)/i, (m) => `Is ${m[3]} something you find yourself still longing for?`],
    [/^do you (.+)/i, (m) => `I'd love to know — ${m[1]}?`],
    [/^are you (happy|okay|fine|content) (with|about) (.+)/i, (m) => `How do you really feel about ${m[3]}, deep down?`],
    [/^are you (.+)/i, (m) => `Would you say you're ${m[1]}?`],
    [/^what (do you think about|are your thoughts on) (.+)/i, (m) => `Where do you land on ${m[2]}, when you sit with it?`],
    [/^what (makes|made) you (.+)/i, (m) => `What is it that draws you to ${m[2]}?`],
    [/^what (.+)/i, (m) => `When you think about it honestly, what ${m[1]}?`],
    [/^how (do you feel about|do you feel) (.+)/i, (m) => `What comes up for you when you think about ${m[2]}?`],
    [/^how (.+)/i, (m) => `I've been curious — how ${m[1]}?`],
    [/^why (.+)/i, (m) => `Help me understand — why ${m[1]}?`],
    [/^when (.+)/i, (m) => `Looking back, when ${m[1]}?`],
    [/^is there (.+)/i, (m) => `Is there ${m[1]} — even something small you've never mentioned?`],
    [/^would you (.+)/i, (m) => `If nothing held you back, would you ${m[1]}?`],
  ];
  for (const [pattern, build] of rewrites) {
    const m = t.match(pattern);
    if (m) return build(m);
  }
  return `Something I've been holding onto: ${t.charAt(0).toLowerCase() + t.slice(1)}?`;
}

async function aiSuggestIntensity(text) {
  await sleep(700);
  const t = text.toLowerCase().trim();
  const spicy = ["kiss", "bed", "naked", "fantasy", "turn on", "attracted", "sexy", "desire", "touch", "physical", "intimate", "body", "want you", "find you", "attraction"];
  const red = ["afraid", "scared", "fear", "insecure", "regret", "hurt", "cry", "vulnerable", "trust", "future together", "children", "marry", "marriage", "love me", "leave", "lose you", "breaking up", "jealous", "therapy", "past relationship", "trauma", "feel loved", "not enough", "settle", "unhappy", "worry about us", "grow apart", "forever", "die", "death", "family", "kids", "moving in", "move in", "long term", "commitment", "argue", "fight", "difficult", "hard for you", "hard for us", "miss", "lonely"];
  const green = ["favourite", "favorite", "happy memory", "made you smile", "laugh", "enjoy", "hobby", "weekend", "food", "music", "film", "movie", "book", "travel", "holiday", "childhood memory", "funny", "proud", "grateful", "morning", "song", "place you love", "best part", "simple", "small thing", "random", "recently"];
  if (spicy.some(w => t.includes(w))) return "spicy";
  if (red.some(w => t.includes(w))) return "red";
  if (green.some(w => t.includes(w))) return "green";
  const wordCount = t.split(/\s+/).length;
  if (wordCount > 14) return "red";
  if (wordCount > 6) return "amber";
  return "amber";
}
...
export default function WonderApp() {
  ...
  const [aiIntensityLoading, setAiIntensityLoading] = useState(false);
  ...
  function handleSkip() {
    setUserSession({ name: userName, coupleCode: null, userId: null });
    setOnboarding("done");
  }
  ...
  async function handlePolish() {
    ...
    setAiError(null);
    try {
      const result = await aiPolish(newQ);
  ...
  async function handleSuggestIntensity(questionText) {
    if (!questionText.trim()) return;
    setAiIntensityLoading(true);
    try {
      const suggested = await aiSuggestIntensity(questionText);
      if (["green", "amber", "red", "spicy"].includes(suggested)) {
        setAiIntensitySuggestion(suggested);
        setNewIntensity(suggested);
      }
    } catch (e) {}
    setAiIntensityLoading(false);
  }
  ...
  function handleQuestionChange(e) {
    ...
    if (val.trim().length > 8 && !intensityManuallySet) {
      intensityTimer.current = setTimeout(() => handleSuggestIntensity(val.trim()), 800);
    }
  }
  ...
  async function saveQuestion() {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (userSession?.coupleCode) {
          await db.from("questions").insert({ text: newQ.trim(), intensity: newIntensity, author_id: userSession.userId || "local", couple_code: userSession.coupleCode, discussed: false, skipped: false });
          await loadQuestions(userSession.coupleCode);
        } else {
        ...
        {showAddSheet && (
          <div className="sheet-overlay" onClick={(e) => e.target === e.currentTarget && setShowAddSheet(false)}>
            <div className="sheet">
              ...
              <textarea rows={3} placeholder="What have you always wanted to know?" value={newQ} onChange={handleQuestionChange} />

              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: -6, marginBottom: 20 }}>
                <button className="pill-btn" onClick={handlePolish} disabled={!newQ.trim() || aiPolishing} style={{ fontSize: 11, padding: "7px 14px" }}>
                  {aiPolishing ? "polishing…" : "✦ Polish wording"}
                </button>
              </div>

              {aiError && <div style={{ background: COLORS.redLight, color: COLORS.red, borderRadius: 12, padding: "12px 16px", marginTop: -12, marginBottom: 20, fontSize: 12, lineHeight: 1.5 }}>{aiError}</div>}

              {aiPolished && (
                <div className="ai-box" style={{ marginTop: -12, marginBottom: 20 }}>
                  <div className="ai-box-label"><span>✦</span> suggested rephrasing</div>
                  <p className="ai-suggestion-text">{aiPolished}</p>
                  <div className="ai-actions">
                    <button className="ai-action-btn accept" onClick={acceptPolish}>Use this</button>
                    <button className="ai-action-btn" onClick={() => setAiPolished(null)}>Keep mine</button>
                  </div>
                </div>
              )}

              <p className="field-label" style={{ marginBottom: 10 }}>
                How it feels {aiIntensityLoading && <span style={{ fontWeight: 300, textTransform: "none", letterSpacing: 0, fontStyle: "italic" }}>…</span>}
              </p>
              <div className="intensity-picker">
                {Object.entries(INTENSITY).map(([key, val]) => (
                  <div key={key} className={`intensity-option ${newIntensity === key ? "selected" : ""}`}
                    ...
                    onClick={() => { setNewIntensity(key); setIntensityManuallySet(true); }}>
